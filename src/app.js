...
io.on('connection', socket => {
    ...
    socket.on('chat',function(data){
        // roomList.get(socket.room_id).chatsession(data,socket.id);
        io.in(socket.room_id).emit('chat', data);
    });
    socket.on('typing',function(data){
        // roomList.get(socket.room_id).typingsession(data,socket.id);
        socket.to(socket.room_id).emit('typing', data);
    });
    ...
    socket.on('join', ({
        ...
        socket.join(room_id)
        console.log('---user joined--- \"' + room_id + '\": ' + name+" "+type )
        if (!roomList.has(room_id)) {
            return cb({
                error: 'room does not exist'
            })
        }
        roomList.get(room_id).addPeer(new Peer(socket.id, name,type))
        socket.room_id = room_id

        cb(roomList.get(room_id).toJson())

        peerList = roomList.get(socket.room_id).getPeerList();
        io.in(socket.room_id).emit('userList', peerList);
    })

    socket.on('getProducers', (data) => {
        console.log(`---get producers--- name:${roomList.get(socket.room_id).getPeers().get(socket.id).name} ${data.type}`)
        // send all the current producer to newly joined member
        console.log()
        if (!roomList.has(socket.room_id)) return
        let producerList = roomList.get(socket.room_id).getProducerListForPeer(socket.id,data)
    ...
    socket.on('produce', async ({
        ...
        let producer_id = await roomList.get(socket.room_id).produce(socket.id, producerTransportId, rtpParameters, kind,type)
        console.log(`---produce--- type: ${kind} name: ${roomList.get(socket.room_id).getPeers().get(socket.id).name} id: ${producer_id}   ${type}`)
        callback({
            producer_id
        })
    })
